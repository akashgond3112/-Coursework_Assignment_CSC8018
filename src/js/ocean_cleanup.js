...
const userInputData = new Map(); // initialised a map store user input for each item

let totalSum = 0; // initialized a total sum variable.
...
function getHighestAmountOfWaste() {
    ...
    calculateEstimatedPlasticFootPrints(); // calculate the total sum
    updatePlasticWasteData(totalSum); // update the DOM with total sum
}

/* 
Calculate the estimated footprint , we will get all input fields value those are non zero
...
function calculateEstimatedPlasticFootPrints() {
    ...
    if (getNumberOfPeople() > 1) {
        calculateDataBasedOnNumberOfPeople();
    }
}
...
document.addEventListener("DOMContentLoaded", function () {
    for (let i = 0; i < inputFields.length; i++) {
        // Once user click on the input field set clear the input field
...
function validateInputFields(event) {
    return event.target.value >= 0 && event.target.value.length >= 0;
}
...
/* Calculate the totals-um based on the number of value. */
function calculateDataBasedOnNumberOfPeople() {
    updatePlasticWasteData(totalSum / getNumberOfPeople());
}

/* Round the totals-um upto 2 decimal places */
